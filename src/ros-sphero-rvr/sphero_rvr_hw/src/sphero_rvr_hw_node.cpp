...
using namespace sphero_rvr_hw_interface;
...
int main(int argc, char**argv)
{
  ...
  boost::shared_ptr<SpheroRVRHWInterface> sphero_rvr_hw_interface
    (new SpheroRVRHWInterface(nh));
  sphero_rvr_hw_interface->init();

  // Create controller manager
  boost::shared_ptr<controller_manager::ControllerManager> controller_manager;
  controller_manager.reset(new controller_manager::ControllerManager(sphero_rvr_hw_interface.get(), nh));
  ...
  while(ros::ok())
  {
    ...
    // Read
    sphero_rvr_hw_interface->read(elapsed_time);

    // Control
    controller_manager->update(ros::Time::now(), elapsed_time);

    // Write 
    sphero_rvr_hw_interface->write(elapsed_time);

    rate.sleep();
  } // end while
  
  sphero_rvr_hw_interface->disconnect();
