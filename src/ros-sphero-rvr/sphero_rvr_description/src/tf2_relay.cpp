...
/**
 * This relay is used to map from a namespace /<namespace>/tf to the global /tf topic.
 * To avoid ambiguity of tf frames, coming from the same robot description, a prefix will be added to the frame names.
 */
int main(int argc, char ** argv)
{
    ros::init(argc, argv, "tf_relay");
    ros::NodeHandle nh;
    ros::Publisher pub = nh.advertise<tf2_msgs::TFMessage>("/tf", 100);
    ros::Publisher pub_static = nh.advertise<tf2_msgs::TFMessage>("/tf_static", 100, true);

    std::string prefix;
    ros::param::param("~prefix", prefix, std::string(""));

    std::vector<std::string> frame_list;
    ros::param::get("~frames", frame_list);

    if (frame_list.empty()) {
        ROS_WARN("'frames' param is empty, rewriting all frames");
    }

    ros::Subscriber sub = nh.subscribe<tf2_msgs::TFMessage>("tf", 100,
   [prefix, &pub, frame_list](const boost::shared_ptr<const tf2_msgs::TFMessage> message) {
    ...
    ros::Subscriber sub_static = nh.subscribe<tf2_msgs::TFMessage>("tf_static", 100,
